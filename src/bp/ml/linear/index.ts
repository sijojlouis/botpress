...
import { Data, LinearModel, LinearParameters as Parameters } from './typings'

type Serialized = LinearModel & {
  labels_idx: string[]
}

export class Trainer implements sdk.MLToolkit.SVM.LinearTrainer {
  ...
  async train(
    ...
    if (labels.length < 2) {
      throw new Error("Linear can't train on a dataset of only one class")
    }
...
export class Predictor implements sdk.MLToolkit.SVM.LinearPredictor {
  private clf: Linear | undefined
  ...
  constructor(json_model: string) {
    ...
    try {
      ...
      this.clf = new Linear({ kFold: 1 }, model)
    } catch (err) {
  ...
  private async _predictProb(coordinates: number[]): Promise<sdk.MLToolkit.SVM.Prediction[]> {
    const results = await (this.clf as Linear).predictProbabilities(coordinates)
  ...
  private async _predictOne(coordinates: number[]): Promise<sdk.MLToolkit.SVM.Prediction[]> {
    ...
    const results = await (this.clf as Linear).predict(coordinates)
    return [
      {
