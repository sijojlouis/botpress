...
import { Data, LinearConfig, LinearModel, Report } from './typings'
...
export class Linear {
  ...
  constructor(config: Partial<LinearConfig>, model?: LinearModel) {
  ...
  private _restore = (model: LinearModel) => {
    const self = this
  ...
  train = async (dataset: Data[], progressCb: (progress: number) => void): Promise<TrainOutput | undefined> => {
    ...
    const model = self._baseSvm.train(dataset, params)
    progressCb(1)
    const fullModel: LinearModel = { ...model, param: { ...self._config, ...model.param } }
